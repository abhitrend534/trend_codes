...
def create_page_1(today_str, day, pdf, completed, missing, extra):
    ...
    for idx, (name, _, color) in enumerate(columns):
        pdf.set_fill_color(*color)
        font_size = get_max_font_size(pdf, name, col_width, row_height, font_style="B")
    ...
    for i in range(max_rows):
        for idx, (_, col_data, color) in enumerate(columns):
            pdf.set_fill_color(*color)
            cell_text = col_data[i]
            font_size = get_max_font_size(pdf, cell_text, col_width, row_height)
...
if __name__ == "__main__":
    ...
    for date_str in pd.date_range(start=start_date, end=end_date):
        today_str = date_str.strftime("%Y-%m-%d")
